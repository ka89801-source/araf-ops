...
function exportInvoicePDF() {
  ...
  const invoiceData = {
    ...
    payment: document.getElementById('invPayment').value,
...
<head>
<meta charset="UTF-8">
<title>فاتورة ${invoiceData.no}</title>
<style>
body{font-family:'Tajawal','Cairo',sans-serif;background:#EDEBE4;padding:40px;color:#1E2A30}
.invoice{width:920px;min-height:1301px;background:#fff;margin:auto;box-shadow:0 30px 80px -28px rgba(17,37,47,.38);overflow:hidden}
.header{background:linear-gradient(135deg,#11252F,#1B3A4B 55%,#234B5F);color:#fff;padding:42px 50px}
.top{display:flex;justify-content:space-between;align-items:flex-start}
.brand h1{font-size:28px;margin:0;font-weight:900}
.brand small{color:#E4D3AE;letter-spacing:1px}
.tag{text-align:left}
.tag h2{font-size:34px;margin:0;color:#E4D3AE}
.body{padding:38px 50px}
.grid{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:28px}
.card{border:1px solid #E9E7DF;border-radius:14px;padding:18px 20px;background:#FAFAF7}
.row{display:flex;justify-content:space-between;border-bottom:1px dashed #E9E7DF;padding:7px 0;font-size:14px}
.row:last-child{border-bottom:0}
.k{color:#7E8B91}
.v{font-weight:700;text-align:left}
table{width:100%;border-collapse:separate;border-spacing:0;margin-top:20px}
th{background:#1B3A4B;color:#fff;padding:13px;text-align:right}
td{padding:15px;border-bottom:1px solid #E9E7DF}
.amount{text-align:left;direction:ltr;font-weight:700}
.summary{width:320px;margin:30px 0 0 auto}
.total{background:linear-gradient(135deg,#11252F,#234B5F);color:#fff;border-radius:14px;padding:18px 20px;display:flex;justify-content:space-between;font-size:22px;font-weight:900}
.footer{border-top:1px dashed #E9E7DF;margin-top:40px;padding-top:24px;color:#7E8B91;font-size:13px}
@media print{body{background:#fff;padding:0}.invoice{box-shadow:none;width:210mm;min-height:297mm}}
</style>
</head>
<body>
<div class="invoice">
  <div class="header">
    <div class="top">
      <div class="brand">
        <h1>شركة أعراف للمحاماة</h1>
        <small>ARAF LEGAL OS</small>
      </div>
      <div class="tag">
        <div>INVOICE · فاتورة إلكترونية</div>
        <h2>فاتورة</h2>
        <div>رقم الفاتورة: <b>${invoiceData.no}</b></div>
      </div>
    </div>
  </div>
  ...
  <div class="body">
    <div class="grid">
      <div class="card">
        <div class="row"><span class="k">اسم العميل</span><span class="v">${invoiceData.customer}</span></div>
        <div class="row"><span class="k">رقم التواصل</span><span class="v">${invoiceData.phone}</span></div>
        <div class="row"><span class="k">البريد الإلكتروني</span><span class="v">${invoiceData.email || '—'}</span></div>
      </div>
      <div class="card">
        <div class="row"><span class="k">تاريخ الإصدار</span><span class="v">${invoiceData.date}</span></div>
        <div class="row"><span class="k">طريقة الدفع</span><span class="v">${invoiceData.payment}</span></div>
        <div class="row"><span class="k">حالة الطلب</span><span class="v">${invoiceData.status}</span></div>
      </div>
    </div>

    <table>
      <thead>
        <tr><th>نوع الخدمة ووصفها</th><th>الكمية</th><th>المبلغ</th></tr>
      </thead>
      <tbody>
        <tr>
          <td>
            <b>${invoiceData.service}</b>
            <div style="color:#7E8B91;font-size:12px;margin-top:6px;line-height:1.7">${invoiceData.desc}</div>
          </td>
          <td>1</td>
          <td class="amount">${invoiceData.amount} ر.س</td>
        </tr>
      </tbody>
    </table>

    <div class="summary">
      <div class="row"><span class="k">المبلغ</span><span class="v">${invoiceData.amount} ر.س</span></div>
      <div class="row"><span class="k">ضريبة القيمة المضافة</span><span class="v">${invoiceData.vat} ر.س</span></div>
      <div class="total"><span>الإجمالي</span><span>${invoiceData.total} ر.س</span></div>
    </div>

    <div class="footer">
      شكرًا لاختياركم أعراف — منصة قانونية سعودية.
    </div>
  </div>
</div>
